...
**Estado atual (2026-09-24):** o piloto inicial de 18 pares foi concluído e adjudicado. A expansão documental deixou 29 pares válidos, agora preparados para uma segunda rodada de anotação humana independente. Esta mesma guideline deve ser usada nas duas rodadas.

Na rodada de expansão, o anotador 1 abre somente `data/annotations/expansion_annotator_1.csv`, e o anotador 2 abre somente `data/annotations/expansion_annotator_2.csv`. Não abram o arquivo um do outro nem `expansion_semantic_pilot_reference.csv` durante a anotação. Cada arquivo contém os mesmos 29 `pair_id`, com ordem das linhas e apresentação A/B independentes. Preencham somente as 17 colunas de resposta descritas abaixo.
...
No piloto inicial, os arquivos equivalentes foram `semantic_pilot_annotator_1.csv`, `semantic_pilot_annotator_2.csv` e `semantic_pilot_reference.csv`; eles estão concluídos e não devem ser editados. Em ambas as rodadas, **A e B são posições de apresentação, não ordem cronológica**; as datas mostradas são as datas oficiais dos eventos. A correspondência cronológica fica guardada no respectivo arquivo de referência. Cada pessoa deve preencher apenas as colunas de resposta listadas abaixo, sem modificar `pair_id`, evento, data, tema, texto, contexto, evidência ou alerta de cegamento.
