...
Em 2026-09-24, dois modelos hospedados na DeepInfra (Qwen2.5-72B-Instruct, Llama-3.3-70B-Instruct-Turbo) foram avaliados nos 18 pares gold em três condições: end-to-end direto, structured sem gate e structured com gate — as duas últimas derivadas da mesma chamada estruturada, isolando o efeito do gate. O gate corrigiu 10 dos 18 pares em cada modelo (todos gold `INCOMPARABLE` que a comparação direta de stance classificava errado), sem piorar nenhum caso no Qwen e piorando 2 no Llama. O baseline end-to-end empatou ou superou o pipeline estruturado com gate nesta amostra pequena. Os mesmos métodos, já congelados, foram aplicados aos 29 pares da expansão como `label_source=MODEL_PREDICTION` — nunca gold, nunca avaliado por accuracy. Recomendação: a anotação humana dos 29 continua necessária. Veja `docs/automatic_experiments.md`.
